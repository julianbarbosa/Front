saul.directive('inputFile', function(){
	return {
		restrict: 'E',
		require: '?model',
		scope:{
		...
		templateUrl: '../directives/inputFile/inputFile.tpl.html',
		link: function ($scope, element, attrs, ngModel) {
			...
			options.allowedExtensions = $scope.accept?$scope.accept:'pdf,png,jpeg,jpg,xls,xlsx';//Por defecto imágenes y pdf
			...
			options.required = $scope.required=='true';//Por defecto False
			...
			function updateValid(){
				...
				if($scope.model.file!=null){
					...
					if (!(options.regex.test($scope.model.file.name))) {
						...
						$scope.model.$error.msg.push('El archivo debe estar en alguno de estos formatos: '+options.allowedExtensions+'.');
					}else{
