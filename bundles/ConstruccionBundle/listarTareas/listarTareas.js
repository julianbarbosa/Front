saul.config(["$stateProvider",
    function ($stateProvider) {
        $stateProvider.state("listar-tareas", {
            url: "/peticion/listar_tareas",
            views: {
                "main": {
                    controller: "ListarTareasCtrl",
                    templateUrl: "../bundles/ConstruccionBundle/listarTareas/listarTareas.tpl.html"
                }
            }
        });
    }
]).controller("ListarTareasCtrl", ["$scope", "TareaSolicitud",
    function ($scope, TareaSolicitud) {
        $scope.currentPage = 1;
        $scope.numPerPage = 10;
        $scope.maxSize = 5;
        ...
        $scope.headers = [
            {type: 'action', name: 'acciones', title: 'Acciones', align: 'text-center', input: false,
                values: [
                    {
                        'icon': '<i class="ver-detalle fa fa-external-link" title="Clic para ver la información completa del expediente"></i>'},
                    {'icon': '<i class="ver-detalle fa fa-history" title="Clic para ver el historial"></i>','link': '#!/expediente/listar_historial/'},
                    {'icon': '<i class="ver-detalle fa fa-hand-paper-o" title="Relizar Proceso Manual"></i>', 'link': '#!/expediente/proceso_manual/'}
                ],
                options: [
