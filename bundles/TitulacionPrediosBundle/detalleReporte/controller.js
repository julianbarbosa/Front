saul.config(["$stateProvider",
    function ($stateProvider) {
        $stateProvider.state("titulacionpredios-generadorreportesfiltro", {
            url: "/titulacion-predios/reporte/:id",
            views: {
                "main": {
                    controller: "TitulacionPrediosGeneradorReportesFiltroCtrl",
                    templateUrl: "../bundles/TitulacionGratuitaBundle/generadorDeReportesFiltro/template.tpl.html"
                }
            }            
        });
    }
]).controller("TitulacionPrediosGeneradorReportesFiltroCtrl", ["$scope", "$http", "$stateParams", "root", "LoadingOverlap", "UtilForm",
function ($scope, $http, $stateParams, root, LoadingOverlap, UtilForm) {
    ...
    $scope.obtenerReportes = function() {
        ...
        $http.get(root+"dominio/reporte-titulacion-gratuita/"+$stateParams.id).then(function(response) {
    ...
    $scope.generarReporte = function(arrayData) {
        ...
        UtilForm.openWith(
            root + "reporte-solicitud/generar",
            'post',jsonData);
    }
