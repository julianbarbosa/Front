saul.config(["$stateProvider",
    function ($stateProvider) {
        $stateProvider.state("certificadonomenclatura-crearsolicitud", {
            url: "/nomenclatura/solicitud",
            views: {
                "main": {
                    controller: "CrearPeticionNomenclaturaCtrl",
                    templateUrl: "../bundles/NomenclaturaBundle/crearPeticion/crearPeticion.tpl.html"
                }
            }
        });
    }
]).controller("CrearPeticionNomenclaturaCtrl", ["$scope", "$state", "root", "urlPlugInNomenclatura","Nomenclatura", "Comuna", "Barrio", "Predio", "Usuario", "dsJqueryUtils", "$uibModal", "Upload",
    function ($scope, $state, root, urlPlugInNomenclatura, Nomenclatura, Comuna, Barrio, Predio, Usuario, dsJqueryUtils, $uibModal, Upload) {        
        alert('Está intentando usar la versión antigua de la solicitud. Lo redireccionaremos a la nueva versión.');
        $state.go('solicitar-certificacion', { tipoUsuario: 'ciudadano'});
        return;
        ...
        $scope.save = function () 
        {
            ...
            if (validate && files && files.length) {                
                dsJqueryUtils.goTop();
                Upload.upload({
                    url: root+'certificado_nomenclatura/crear_solicitud/',
                    data: {
                        data: $scope.peticion,
