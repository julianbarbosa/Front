saul.config(["$stateProvider",
    function ($stateProvider) {
        $stateProvider.state("generadorreportesfiltro", {
            url: "/solicitud/reporte/:id",
            views: {
                "main": {
                    controller: "GeneradorReportesFiltroCtrl",
                    templateUrl: "../bundles/ControlPolicivoBundle/generadorDeReportesFiltro/template.tpl.html"
                }
            }            
        });
    }
]).controller("GeneradorReportesFiltroCtrl", ["$scope", "$http", "$stateParams", "root", "LoadingOverlap", "UtilForm",
function ($scope, $http, $stateParams, root, LoadingOverlap, UtilForm) {
    ...
    $scope.obtenerReporte = function() {
        ...
        $http.get(root+"dominio_id/"+$stateParams.id).then(function(response) {
