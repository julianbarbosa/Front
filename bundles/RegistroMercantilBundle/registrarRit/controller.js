saul.config(["$stateProvider",
    function ($stateProvider) {
        $stateProvider.state("rit-registro", {
            url: "/rit/registro",
            views: {
                "main": {
                    controller: "RegistroMercantilSolicitudCtrl",
                    templateUrl: "../bundles/RegistroMercantilBundle/registrarRit/template.tpl.html"
                }
            }            
        });
    }
]).controller("RegistroMercantilSolicitudCtrl", ["$scope", "$http", "root", "LoadingOverlap","PluginNomenclatura", "ListarTiposDocIdentificacion","UsuarioActual", "Upload",
    function ($scope, $http, root, LoadingOverlap, PluginNomenclatura, ListarTiposDocIdentificacion, UsuarioActual, Upload) {
        ...
        $scope.registro.arrayRegimenRIT = [];
        $scope.registro.arrayNaturalezaRIT = [];
        ...
        $scope.obtenerRegimenRIT = function() {
            $http.get(root+"dominio/regimenRIT").then(function(response) {
                $scope.arrayRegimenRIT = response.data;
            });
        }
        $scope.obtenerRegimenRIT();
        
        $scope.obtenerNaturalezaRIT = function() {
            $http.get(root+"dominio/naturalezaRIT").then(function(response) {
                $scope.arrayNaturalezaRIT = response.data;
            });
        }
        $scope.obtenerNaturalezaRIT();
