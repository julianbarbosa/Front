saul
  .config([
    ...
    function ($stateProvider) {
      $stateProvider.state("crearsolicitud-visitas", {
        url: "/solicitudesvisitas/crearsolicitud",
        views: {
          main: {
            controller: "AsignarVisitaControlInfraestructuraCtrl",
            templateUrl:
              "../bundles/ControlUrbanoBundle/crearsolicitud/crearsolicitud.tpl.html",
          },
        },
      });
    },
  ...
  .controller("AsignarVisitaControlInfraestructuraCtrl", [
    ...
    function ($scope, $http, LicenciaAll, LoadingOverlap, $stateParams, root) {
      ...
      $scope.headers = [
        {
          ...
          values: [
            {
              icon: '<i class="ver-detalle fa fa-pencil-square-o  pull-left" title="Crear Solicitud de Visita Control Posterior"></i>',
              action: "crearSolicitudVisitaControlPosterior",
            },
            {
              icon: '<i class="ver-detalle fa fa-plus-square pull-left" title="Ver Detalle de la licencia"></i>',
              style: "text-danger",
              action: "activarDetalle",
            },
          ],
        },
        {
      ...
      $scope.consultarCuradurias = function () {
        $http
          .get(root + "app_dev.php/licencias/curaduria/")
          .then(function (data) {
            console.log(data);
      ...
      function callback(data) {
        ...
        $scope.validator.alert.content =
          data.msg +
          ' <a href="#!/controlpolicivo/verificarlicencia" ><i class="fa fa-file-text" aria-hidden="true"></i> Volver a lista</a>';
      ...
      $scope.actualizarDatos = function (page) {
        ...
        request.push({
          name: "identificacionpropietario",
          value: $scope.busqueda.identificacionpropietario,
        });
        request.push({
          name: "propietario",
          value: $scope.busqueda.nombrepropietario,
        });
        ...
        request.push({
          ...
          value: $scope.busqueda.matriculainmobiliaria,
        });
        LicenciaAll.get(request).$promise.then(function (response) {
      ...
      $scope.crearSolicitudVisitaControlPosterior = function (item) {
        bootbox.confirm(
          "Confirma que desea crear una visita de control posterior para el predio ubicado en la dirección " +
            item.direccion +
            "?",
          function (result) {
            if (result) {
              ...
              $http
                .post(root + "controlposterior/crearsolicitudvisita/" + item.id)
