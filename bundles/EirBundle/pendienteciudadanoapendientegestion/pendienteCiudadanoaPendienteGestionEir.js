saul.config(["$stateProvider",
	function ($stateProvider) {
		$stateProvider.state('pendienteciudadanoapendientegestion', {
			url: '/eir/pendienteciudadanoapendientegestion/{idSolicitud}',
			views: {
				main: {
					controller: 'PendienteCiudadanoaPendienteGestionEirCtrl',
					templateUrl: '../bundles/EirBundle/pendienteciudadanoapendientegestion/pendienteCiudadanoaPendienteGestionEir.tpl.html'
				}
			}
		});
		...
		.controller('PendienteCiudadanoaPendienteGestionEirCtrl',
				['$scope', '$window', '$resource', '$state', '$stateParams', '$filter', '$uibModal', 'MostrarSolicitudEir', 'ConsultarSolicitante', 'Comuna', 'root', 'ConfirmarDatosEir', 'Upload', 'ListarActividadesCiiuSolicitadas', 'PendienteCiudadanoaPendienteGestionEir',
					function ($scope, $window, $resource, $state, $stateParams, $filter, $modal, MostrarSolicitudEir, ConsultarSolicitante, ComunaFactory, root, ConfirmarDatosEir, Upload, ServicioListarActividadesCiiuSolicitadas, PendienteCiudadanoaPendienteGestionEir) {
						...
						$scope.init = function (idSolicitud) {
							$scope.estado = 'pendienteciudadanoapendientegestion';
							$scope.messageErrorCliente = '';
							$scope.solicitud = {idsolicitud: $stateParams.idSolicitud};

							PendienteCiudadanoaPendienteGestionEir.validaTiposolicitud($stateParams.idSolicitud).then(
									function (response) {
										$scope.errors = [];
						...
						$scope.guardarSolicitud = function () {
							...
							var allData = {
								...
								idclasevisitaseguridadyjusticia: $scope.solicitud.idclasevisitaseguridadyjusticia,
							...
							Upload.upload({
								url: root + 'eir/pendienteciudadanoapendientegestion/cambiarciudadanoapendientegestion',
									...
									function (resp) {
										...
										$(".overlap_espera_1").fadeOut(500, "linear", function () {
											...
											if (response.success) {
												...
												bootbox.alert('Se ha cambio correctamente el estado de la solicitud: ' + response.data.idsolicitud + ', a pendiente Gestión EIR.',
														function (result) {
