saul.config(["$stateProvider",
	function ($stateProvider) {
		$stateProvider.state('visitaseguridadyjusticiaeir', {
			url: '/eir/visitaseguridadyjusticia/{idSolicitud}',
			views: {
				main: {
					controller: 'VisitaSeguridadyJusticiaEirCtrl',
					templateUrl: '../bundles/EirBundle/visitaseguridadyjusticia/visitaSeguridadyJusticiaEir.tpl.html'
				}
			}
		});
		...
		.controller('VisitaSeguridadyJusticiaEirCtrl',
				['$scope', '$window', '$resource', '$state', '$stateParams', '$filter', '$uibModal', 'MostrarSolicitudEir', 'ConsultarSolicitante', 'Comuna', 'root', 'ConfirmarDatosEir', 'Upload', 'ListarActividadesCiiuSolicitadas', 'VisitaSeguridadyJusticiaEir',
					function ($scope, $window, $resource, $state, $stateParams, $filter, $modal, MostrarSolicitudEir, ConsultarSolicitante, ComunaFactory, root, ConfirmarDatosEir, Upload, ServicioListarActividadesCiiuSolicitadas, VisitaSeguridadyJusticiaEir) {
						...
						$scope.init = function (idSolicitud) {
							$scope.estado = 'visitaseguridadyjusticia';
							$scope.messageErrorCliente = '';
							$scope.solicitud = {idsolicitud: $stateParams.idSolicitud};

							VisitaSeguridadyJusticiaEir.validaTiposolicitud($stateParams.idSolicitud).then(
									function (response) {
										$scope.errors = [];
						...
						$scope.guardarSolicitud = function () {
							...
							Upload.upload({
								url: root + 'eir/visitaseguridadyjusticia/crearVisitaSeguridadyJusticia',
